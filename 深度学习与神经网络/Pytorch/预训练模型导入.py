...
class MobileNetV2(nn.Module):
	def __init__(self, num_classes=1000, alpha=1.0, round_nearest=8, keep_rate=0.2):
		...
		features = []
		features.append(ConvBNReLU(3, in_channels, stride=1))
		...
		features.append(ConvBNReLU(in_channels, last_channels, 1))
...
if True:
	...
	for param in nnw.features.parameters():
		...
		pass
	i = 0
	# for t in pre_weights:
	# 	print(t, pre_weights[t].size())
	# 	i += 1
	# 	if (i == 20):
	# 		break;
	for t in missing_keys:
		print(t, nnw.state_dict()[t].size())
		i += 1
		if (i == 6):
			break
	for t in unexpected_keys:
		print(t, pre_weights[t].size())
		if (i == 6):
			break
else:
