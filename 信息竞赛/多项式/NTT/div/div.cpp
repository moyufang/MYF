...
struct Poly{
	...
	Poly &div(const Poly &d, Poly &q, Poly &r){
		...
		(r = q).mul(d).resize(d.n-1);
		_for(i, 0, r.n) r.a[i] = (a[i]+mod-r.a[i])%mod; 
		return *this;
	}
}p, d, q, r;
...
int main(){
	...
	re int n = p.n = rdnt()+1, m = d.n = rdnt()+1;
	init_ntt(1<<lb(n-1)+2);
	_for(i, 0, p.n) p.a[i] = rdnt(); _for(i, 0, d.n) d.a[i] = rdnt();
	p.div(d, q, r);
	q.resize(n-m+1); r.resize(m-1); 
	print(q); print(r);
//	q.mul(d);
//	_for(i, 0, p.n) assert(((i<r.n?r.a[i]:0)+q.a[i])%mod == p.a[i]);
		
//	unsigned long long tmp = 0;
//	_for(i, 0, p.n) tmp = tmp*mod+p.a[i];
//	printf("%llu\n", tmp);
