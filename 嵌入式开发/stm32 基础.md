...
stm32 标准将 `4GB` 空间划分成 `8` 个 `512MB` 的块，如图
![](stm32_image/stm32存储器映射结构.png)
其中，外设寄存器分布在 `Block1` 和 `Block2` 上，在 `Blockx` 上，低 `1MB` 和 高 `32MB` 分别称作“位带区"和"位带别名区"，访问寄存器通过”位带区"，通过别名访问寄存器的一位通过"位带别名区"，”位带区“中某一位到”位带别名区“的换算公式如下
```
(1)位带别名区地址 = 位带别名区基地址 + 位带别名区偏移地址
(2)位带别名区偏移地址 = (位带区偏移地址 * 8 + 位序号) * 4
```
ST 公司提供的标准库中，对寄存器的访问的底层实现即是"位带操作"。
### 时钟配置
...
两种输出模式还有一个“复用”词条，指代的模式下GPIO的输出行为由ADC、TIM等外设控制而非CPU的ODR寄存器控制
